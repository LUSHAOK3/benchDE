# Figure 4: Poisson standard-simulation AUC.

set.seed(8848)

BASE_DIR <- "/public/workspace/fangzhaoyuan/lsk/plos/MultiDimDE"
AUC_DIR  <- "/public/workspace/fangzhaoyuan/lsk/plos/MultiDimDE/poisson/auc"

timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
OUT_DIR <- file.path(BASE_DIR, "figures", paste0("Fig4_", timestamp))
...
write.csv(auc_long, file.path(OUT_DIR, "Fig4_data.csv"), row.names = FALSE)
...
write.csv(auc_summary, file.path(OUT_DIR, "Fig4_summary.csv"), row.names = FALSE)
...
write.csv(auc_with_outlier, file.path(OUT_DIR, "Fig4_data_with_outlier_flag.csv"), row.names = FALSE)
write.csv(auc_with_outlier[auc_with_outlier$is_outlier, ],
          file.path(OUT_DIR, "Fig4_outliers.csv"),
          row.names = FALSE)
...
title_grob <- cowplot::ggdraw() +
  cowplot::draw_label("Poisson-based standard simulation AUC results", fontface = "bold", size = 11, x = 0.02, hjust = 0)

fig_final <- cowplot::plot_grid(title_grob, fig, ncol = 1, rel_heights = c(0.045, 1))

pdf_file  <- file.path(OUT_DIR, "Fig4.pdf")
tiff_file <- file.path(OUT_DIR, "Fig4.tiff")
png_file  <- file.path(OUT_DIR, "Fig4.png")
...
ggsave(file.path(OUT_DIR, "Fig4_small.pdf"),
       fig_final, width = 5.8, height = 6.4, units = "in")
ggsave(file.path(OUT_DIR, "Fig4_small.tiff"),
       fig_final, width = 5.8, height = 6.4, units = "in", dpi = 600, compression = "lzw")
ggsave(file.path(OUT_DIR, "Fig4_small.png"),
