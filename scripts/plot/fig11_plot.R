# Figure 11: SCLC CAT stability.

set.seed(8848)

BASE_DIR <- Sys.getenv("BENCHDE_DIR", unset = "/public/workspace/fangzhaoyuan/lsk/plos/MultiDimDE")
DATASET_LABEL <- Sys.getenv("SCLC_DATASET_LABEL", unset = "SCLC")
CONC_DIR <- Sys.getenv("SCLC_CAT_CONC_DIR", unset = file.path(BASE_DIR, "CAT_SCLC", "conc"))

NSAMPLE <- as.integer(strsplit(Sys.getenv("SCLC_PLOT_NSAMPLE", unset = "3,5,10,30,50"), ",")[[1]])

timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
OUT_DIR <- file.path(BASE_DIR, "figures", paste0("Fig11_", timestamp))
dir.create(OUT_DIR, recursive = TRUE, showWarnings = FALSE)

if (!dir.exists(CONC_DIR)) {
  stop("CONC_DIR does not exist: ", CONC_DIR,
       "\nPlease check whether CAT_SCLC/conc has been generated.")
}
...
read_conc_one_n <- function(n) {
  f <- file.path(CONC_DIR, sprintf("%s-1vs2.rds", n))
  if (!file.exists(f)) {
    warning("Missing conc file: ", f)
    return(data.frame())
  }
  ...
  if (is.data.frame(x)) {
    dat <- x
    dat$iteration <- if ("iteration" %in% colnames(dat)) dat$iteration else 1L
  } else if (is.list(x)) {
    parts <- list()
    ...
    if (length(parts) == 0) {
      warning("No data.frame elements found in: ", f)
      return(data.frame())
    }
  ...
  if (!"conc" %in% cn) {
    alt <- intersect(c("Concordance", "concordance", "CAT", "cat"), cn)
    if (length(alt) > 0) {
      dat$conc <- dat[[alt[1]]]
    }
  }
  ...
  if (!"method1" %in% colnames(dat)) {
    alt <- intersect(c("Method", "method", "methods"), colnames(dat))
  ...
  if (!"rank" %in% colnames(dat)) {
    dat$rank <- NA_integer_
  }

  if (!all(c("method1", "method2", "conc") %in% colnames(dat))) {
    warning("Conc file does not contain required columns method1/method2/conc: ", f)
    return(data.frame())
  }
...
write.csv(all_conc, file.path(OUT_DIR, "Fig11_data.csv"), row.names = FALSE)

cat_summary <- aggregate(conc ~ Dataset + nSample + method1, all_conc, function(x) {
  c(
    n = length(x),
...
write.csv(cat_summary, file.path(OUT_DIR, "Fig11_summary.csv"), row.names = FALSE)
...
make_one_panel <- function(n) {
  ...
  dat$method1_chr <- as.character(dat$method1)

  if (nrow(dat) == 0) {
    return(ggplot() + theme_void() + ggtitle(paste0("n = ", n)))
  }

  med <- aggregate(conc ~ method1_chr, dat, median, na.rm = TRUE)
  med <- med[order(med$conc, decreasing = FALSE), , drop = FALSE]
  dat$method_plot <- factor(dat$method1_chr, levels = med$method1_chr)

  ggplot(dat, aes(x = method_plot, y = conc, color = method1_chr)) +
    geom_boxplot(
      outlier.size = 0.65,
      outlier.alpha = 0.80,
      linewidth = 0.28,
...
legend_dat$method1_chr <- as.character(legend_dat$method1)

g_legend <- ggplot(legend_dat, aes(x = method1_chr, y = conc, color = method1_chr)) +
...
pdf_file <- file.path(OUT_DIR, "Fig11.pdf")
tiff_file <- file.path(OUT_DIR, "Fig11.tiff")
png_file <- file.path(OUT_DIR, "Fig11.png")
