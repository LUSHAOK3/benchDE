# Figure 5: Poisson standard-simulation FDR-TPR curves.

set.seed(8848)

BASE_DIR <- Sys.getenv("BENCHDE_DIR", unset = "/public/workspace/fangzhaoyuan/lsk/plos/MultiDimDE")
DATP_DIR <- Sys.getenv("POISSON_STANDARD_DATP_DIR", unset = "/public/workspace/fangzhaoyuan/lsk/plos/MultiDimDE/poisson/datp")

timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
OUT_DIR <- file.path(BASE_DIR, "figures", paste0("Fig5_", timestamp))
...
read_all_curves <- function(datp_dir) {
  ...
  if (length(skipped) > 0) {
    write.csv(do.call(rbind, skipped), file.path(OUT_DIR, "Fig5_skipped.csv"), row.names = FALSE)
  }
...
write.csv(curve_long, file.path(OUT_DIR, "Fig5_data.csv"), row.names = FALSE)
...
write.csv(curve_summary, file.path(OUT_DIR, "Fig5_summary.csv"), row.names = FALSE)
...
p <- ggplot(curve_summary, aes(x = FDR, y = mean, color = Method, group = Method)) +
  ...
  labs(
    title = "Poisson-based standard simulation FDR-TPR results",
    x = "FDR",
    y = "TPR",
    color = "Methods"
  ) +
...
ggsave(file.path(OUT_DIR, "Fig5.pdf"), p, width = 8.2, height = 8.8, units = "in", limitsize = FALSE)
ggsave(file.path(OUT_DIR, "Fig5.tiff"), p, width = 8.2, height = 8.8, units = "in", dpi = 600, compression = "lzw", limitsize = FALSE)
ggsave(file.path(OUT_DIR, "Fig5.png"), p, width = 8.2, height = 8.8, units = "in", dpi = 300, limitsize = FALSE)
