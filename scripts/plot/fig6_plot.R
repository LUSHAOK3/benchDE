...
OUT_DIR <- file.path(BASE_DIR, "figures", paste0("Fig6_", timestamp))
...
write.csv(auc_long, file.path(OUT_DIR, "Fig6_data.csv"), row.names = FALSE)
...
write.csv(auc_summary, file.path(OUT_DIR, "Fig6_summary.csv"), row.names = FALSE)
...
write.csv(slope_df, file.path(OUT_DIR, "Fig6_slope.csv"), row.names = FALSE)
...
pdf_file <- file.path(OUT_DIR, "Fig6.pdf")
...
if (SAVE_TIFF) {
  if (!requireNamespace("ragg", quietly = TRUE)) {
    ...
    tiff_file <- file.path(OUT_DIR, "Fig6.tiff")
    ragg::agg_tiff(
      filename = tiff_file,
      width = 6.68,
      height = 7.20,
