...
create_countmat <- function(param_list, gs, dispType = dispType, mode = mode, random_sampling = FALSE, 
                            s = s, n.var = 10000, RO.prop = RO.prop, Large_sample = FALSE, fixedfold = FALSE){
  ...
  if(dispType == 'different'){
    sample.disp1 <- sapply(sample.mean1, FUN = getDisp, mean.condition = mean.c1, disp.condition = disp.c1, simplify = T, USE.NAMES = F)
    sample.disp2 <- sapply(sample.mean2, FUN = getDisp, mean.condition = mean.c2, disp.condition = disp.c2, simplify = T, USE.NAMES = F)
...
simulateOneSplit <- function(X, rseed, param = param, RO.prop = RO.prop, s = s, n.var = 10000){
  ...
  dat <- create_countmat(param, gs, s = s, dispType = 'different', n.var = n.var, RO.prop = RO.prop, mode = "R")
}
